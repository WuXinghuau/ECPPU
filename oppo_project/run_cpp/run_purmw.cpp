...
int main(int argc, char **argv)
{
  ...
  std::cerr<<"11\n";
  std::string new_v1;
  OppoProject::random_generate_value(new_v1,176*2);
  std::cout<<"kill some node no more than  2 \n and input an int : ";
  int input_ready;
  std::cin>>input_ready;
  std::cout<<"start to update \n";
  if(client.update(k1,0,new_v1.length(),new_v1,up_func)) std::cout<<"test update success\n";
  else std::cout<<"test update failed\n";
